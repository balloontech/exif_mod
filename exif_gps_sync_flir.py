"""
Module for extracting GPS data from MicroLoon CSV files and embedding it into FLIR image EXIF metadata.
...
Functions:
    ...
    - read_filename_timestamp(folder_path): Reads filenames and corresponding timestamps from image files.
...
def read_filename_timestamp(folder_path):
    data = []
    jpg_files = glob.glob(os.path.join(folder_path, '*.jpg'))
    for file_path in jpg_files:
        try:
            filename = os.path.basename(file_path)
            date_str, time_str = filename.split('_')[:2]
            date_obj = datetime.strptime(date_str, '%Y%m%d')
            time_obj = datetime.strptime(time_str, '%H%M%S')
            datetime_obj = datetime.combine(date_obj.date(), time_obj.time())
            data.append((file_path, datetime_obj))
        except ValueError:
            print(f"Error: Unable to parse datetime from filename {filename} in folder {folder_path}")
            return None
    return data
...
def main():
    ...
    photo_data = read_filename_timestamp(photo_path)
